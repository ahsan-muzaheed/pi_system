...
$ProcessExe = "node.exe"
$Arguments = @("pie_agent", "--peerConnectionOptions=""$peerConnectionOptions""", "--publicIp=$PublicIp")
# Add arguments passed to script to Arguments for executable
$Arguments += $args

Write-Output "Running: $ProcessExe $Arguments"
node pie_agent --peerConnectionOptions=$peerConnectionOptions --publicIp=127.0.0.1
